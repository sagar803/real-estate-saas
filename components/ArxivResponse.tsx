      ...
      {papers.map((paper, index) => (
        <Card key={index} className="mb-4">
          ...
          <CardFooter className='flex gap-2'>
            <Button><a target='_blank' href={paper.id}>View on arXiv</a></Button>
            {paper.links.pdf.length > 0 && <Button onClick={() => setSelectedPdfUrl(paper.links.pdf[0])}>View pdf</Button>}
          </CardFooter>
        </Card>
      ))}
    </div>
