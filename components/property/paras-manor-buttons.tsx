...
import { ImageIcon, Camera, Photos, Video, Play, Building } from 'lucide-react';
...
export const ParasManorDetailsButton = () => {
  const [, setMessages] = useUIState<typeof AI>();
  const { submitUserMessage } = useActions<typeof AI>();
  
  const handleSubmit = async (event: React.MouseEvent) => {
    const query = `call this function - show_paras_manor_property`;
    event.preventDefault();
    const response = await submitUserMessage(query);
    setMessages(currentMessages => [...currentMessages, response]);
  }
