...
export function DateSelect() {
  ...
  const query = `the selected date is ${calculatePastDate(selectedDate)}, now display the research papers`;
      ...
      <div className="space-y-2">
        <div className="mt-2 max-h-60 overflow-auto">
          {predefinedRanges.map(range => (
            <div
              key={range}
              className={`m-1 cursor-pointer p-2 hover:bg-gray-200 border rounded-lg ${selectedDate == range ? 'bg-gray-200' : ''}`}
